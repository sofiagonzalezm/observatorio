...
with tab1:
    ...
    st.dataframe(muni_display, use_container_width=True, 
                 column_config={
            "monto_total_ejecutado": st.column_config.NumberColumn(
            "Monto total ejecutado",
            format="%.2f"), 
            "promedio_monto_ejecutado": st.column_config.NumberColumn(
            "Monto Promedio ejecutado",
            format="%.2f" ), 
    ...
    if municipio_sel:
        ...
        k4.metric("Total monto ejecutado", f"{monto_total_ejecutado:,.0f}")
        ...
        for col in ["monto_adjudicado", "monto_ejecutado", "brecha_adjudicado_ejecutado", "meta_ejecutada"]:
            if col in detalle_display.columns:
                detalle_display[col] = detalle_display[col].map(
                    lambda x: f"{x:,.0f}" if pd.notna(x) else ""
                )

        #if "ratio_meta_ejecutada" in detalle_display.columns:
         #   detalle_display["ratio_meta_ejecutada"] = detalle_display["ratio_meta_ejecutada"].map(
        ...
        st.dataframe(styled_df, use_container_width=True, 
                     column_config={
            "meta_ejecutada": st.column_config.NumberColumn(
            "Meta ejecutada",
            format="%.2f"), 
...
with tab2:
    ...
    # formatear
    for col in ["proyectos", "monto_total_ejecutado", "promedio_monto_ejecutado", "proyectos_meta0_gasto", "proyectos_sobreejecucion"]:
        if col in mayor_one_display.columns:
            mayor_one_display[col] = mayor_one_display[col].map(lambda x: f"{x:,.0f}" if pd.notna(x) else "")
    ...
    st.dataframe(mayor_one_display, use_container_width=True)
    ...
    if alcalde_sel:
        ...
        k1, k2, k3 = st.columns(3)
        k1.metric("Total proyectos", f"{total_proyectos:,.0f}")
        k2.metric("Monto total ejecutado", f"{monto_total_ejecutado:,.0f}")
        k3.metric("Promedio ratio meta ejecutada", f"{ratio_promedio:.2%}" if pd.notna(ratio_promedio) else "")

        k4, k5, k6 = st.columns(3)
        k4.metric("Municipio", municipio_val)
        k5.metric("Departamento", departamento_val)
        k6.metric("% de proyectos sospechosos", f"{pct_sospechosos:.2%}" if pd.notna(pct_sospechosos) else "")

        k7, k8 = st.columns(2)
        k7.metric("% con meta física = 0 y gasto", f"{pct_sin_meta_ejecutada:.2%}" if pd.notna(pct_sin_meta_ejecutada) else "")
...
df_filtered["snip"] = df_filtered["snip"].astype(str).str.strip()
coordenadas["snip"] = coordenadas["snip"].astype(str).str.strip()
df_filtered = df_filtered.merge(
    coordenadas[["snip", "latitud", "longitud"]],
    on="snip",
    how="left"
)
...
with tab3:
    ...
    if "latitud" in df_sospechosos.columns and "longitud" in df_sospechosos.columns and not df_sospechosos.empty:
        mapa_sos = df_sospechosos.groupby(
            ["municipio", "departamento", "latitud", "longitud"]
        ).size().reset_index(name="num_sospechosos")

        fig = px.scatter_mapbox(
            mapa_sos,
            lat="latitud",
            lon="longitud",
            size="num_sospechosos",
            color="num_sospechosos",
        ...
        st.warning("⚠️ No hay coordenadas o no existen proyectos sospechosos para mostrar el mapa.")

    # -------------------------
    # MAPA: meta ejecutada = 0 pero con gasto
    # -------------------------
    ...
    if "latitud" in df_meta0.columns and "longitud" in df_meta0.columns and not df_meta0.empty:
        mapa_df = (
            df_meta0.groupby(["municipio", "departamento", "latitud", "longitud"])
            .size()
        ...
        fig = px.scatter_mapbox(
            ...
            lat="latitud",
            lon="longitud",
            size="num_meta0_con_gasto",
            color="num_meta0_con_gasto",
            hover_name="municipio",
            hover_data={
                "departamento": True,
        ...
        st.warning("⚠️ No hay coordenadas o no existen proyectos con meta ejecutada = 0 y gasto para mostrar el mapa.")
...
with tab4:
    ...
    partidos_display["proyectos"] = partidos_display["proyectos"].map(lambda x: f"{x:,.0f}")
    partidos_display["alcaldes"] = partidos_display["alcaldes"].map(lambda x: f"{x:,.0f}")
    partidos_display["proveedores"] = partidos_display["proveedores"].map(lambda x: f"{x:,.0f}")
    partidos_display["monto_adjudicado"] = partidos_display["monto_adjudicado"].map(lambda x: f"{x:,.0f}")
    partidos_display["monto_ejecutado"] = partidos_display["monto_ejecutado"].map(lambda x: f"{x:,.0f}")
    partidos_display["ratio_promedio"] = partidos_display["ratio_promedio"].map(lambda x: f"{x:.2%}" if pd.notna(x) else "")
    partidos_display["pct_sospechosos"] = partidos_display["pct_sospechosos"].map(lambda x: f"{x:.2%}" if pd.notna(x) else "")
    partidos_display["pct_meta0_gasto"] = partidos_display["pct_meta0_gasto"].map(lambda x: f"{x:.2%}" if pd.notna(x) else "")
    partidos_display["pct_sobreejecucion"] = partidos_display["pct_sobreejecucion"].map(lambda x: f"{x:.2%}" if pd.notna(x) else "")

    st.dataframe(partidos_display, use_container_width=True)
    ...
    if partido_sel:
        ...
        k1, k2, k3 = st.columns(3)
        k1.metric("Número de proveedores", f"{total_proveedores:,.0f}")
        k2.metric("Monto total ejecutado", f"{monto_total:,.0f}")
        ...
        # formato
        for col in ["monto_adjudicado", "monto_ejecutado"]:
            detalle[col] = detalle[col].map(lambda x: f"{x:,.0f}" if pd.notna(x) else "")

        detalle["ratio_meta_ejecutada"] = detalle["ratio_meta_ejecutada"].map(
            lambda x: f"{x:.2%}" if pd.notna(x) else ""
        )
...
with tab5:
   ...
   # base principal por proveedor
    ...
    prov_display["proyectos"] = prov_display["proyectos"].map(lambda x: f"{x:,.0f}" if pd.notna(x) else "")
    prov_display["municipios"] = prov_display["municipios"].map(lambda x: f"{x:,.0f}" if pd.notna(x) else "")
    prov_display["alcaldes"] = prov_display["alcaldes"].map(lambda x: f"{x:,.0f}" if pd.notna(x) else "")
    prov_display["monto_total_ejecutado"] = prov_display["monto_total_ejecutado"].map(lambda x: f"{x:,.0f}" if pd.notna(x) else "")
    prov_display["promedio_monto_ejecutado"] = prov_display["promedio_monto_ejecutado"].map(lambda x: f"{x:,.0f}" if pd.notna(x) else "")
    prov_display["promedio_ratio_meta_ejecutada"] = prov_display["promedio_ratio_meta_ejecutada"].map(lambda x: f"{x:.2%}" if pd.notna(x) else "")
    ...
    if proveedor_sel:
        ...
        k5.metric("Total monto ejecutado", f"{monto_total_ejecutado:,.0f}")
...
with tab6:
    ...
    proveedores_display["proyectos"] = proveedores_display["proyectos"].map(lambda x: f"{x:,.0f}")
    proveedores_display["costo_promedio"] = proveedores_display["costo_promedio"].map(lambda x: f"{x:,.2f}")
    proveedores_display["costo_mediano"] = proveedores_display["costo_mediano"].map(lambda x: f"{x:,.2f}")

    st.dataframe(proveedores_display, use_container_width=True)
